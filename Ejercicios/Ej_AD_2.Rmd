...
```{r}
#Directamente buscamos en qué posiciones están los TRUE del vector lógico del ejercicio anterior
ind = which(low == TRUE)
ind
```
...
```{r}
#Escogemos de la columna de los estados aquellos dados por la lista de índices del ejercicio anterior
murders$state[ind]
```
...
#Creamos un vector lógico que nos dice, de las entradas de murder_rate, cuáles están por debajo de la tasa media, y utilizamos sum para ver de cuantos estados se trata
...
#De hecho podemos reutilizar la expresión del ejercicio anterior para saber a qué estados corresponden
...
#####NO SÉ SI DEBERÍAMOS APLICARLA SOLO A TOTAL GUN MURDERS O A LAS DOS
```
...
boxplot(population_mill~region, data = murders)
```
