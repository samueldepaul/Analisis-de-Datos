...
data <- as_tibble(data)
```
En el dataset original las variables tienen nombres muy largos y engorrosos por lo que, primero las renombrado y a continuación detallaremos la información que aporta cada una:
```{r, echo = FALSE}
names(data)[1] <- "code"
names(data)[2] <- "age"
names(data)[3] <- "gender"
...
names(data)[7] <- "antidepr"
names(data)[8] <- "antipark"
names(data)[9] <- "antipsych"
names(data)[10] <- "benzodiaz"
```
...
**code**: (???) Simplemente Identificación de cadad paciente.

**age**: (numérica) Edad en años. 

**gender:** (categórica con 2 niveles): Género del paciente.

**history**: (categórica de 2 niveles) Historial familiar de Parkinson.

**Age of disease onset**: (numérica) Edad del paciente al inicio de la enfermedad, en años.
...
**benzodiaz**: (Categórica con 4 niveles): Medicación con benzodiazepinas del paciente.
