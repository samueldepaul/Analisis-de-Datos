...
data$`X20-head` = as.numeric(data$`X20-head`)
data$`X20-RUE` = as.numeric(data$`X20-RUE`)
data$`X20-LUE` = as.numeric(data$`X20-LUE`)
data$`X20-RLE` = as.numeric(data$`X20-RLE`)
data$`X20-LLE` = as.numeric(data$`X20-LLE`)
data$`X21-RUE` = as.numeric(data$`X21-RUE`)
data$`X21-LUE` = as.numeric(data$`X21-LUE`)
data$`X22-neck` = as.numeric(data$`X22-neck`)
data$`X22-RUE` = as.numeric(data$`X22-RUE`)
data$`X22-LUE` = as.numeric(data$`X22-LUE`)
data$`X22-RLE` = as.numeric(data$`X22-RLE`)
data$`X22-LLE` = as.numeric(data$`X22-LLE`)
data$`X23-RUE` = as.numeric(data$`X23-RUE`)
data$`X23-LUE` = as.numeric(data$`X23-LUE`)
data$`X24-RUE` = as.numeric(data$`X24-RUE`)
data$`X24-LUE` = as.numeric(data$`X24-LUE`)
data$`X25-RUE` = as.numeric(data$`X25-RUE`)
data$`X25-LUE` = as.numeric(data$`X25-LUE`)
data$`X26-RLE` = as.numeric(data$`X26-RLE`)
data$`X26-LLE` = as.numeric(data$`X26-LLE`)
...
Pero como podemos ver en el gráfico de la izquierda, no parece que exista tal tendencia. Además, en el gráfico de la derecha hemos mirado a ver si el resultado del test se ve afectado por el tiempo total que llevan los pacientes con dicha enfermedad y tampoco parece existir ninguna relación.
\newpage 
Tenemos un conjunto de variables ($X18, \ldots ,X31$) que se corresponden con las diferentes pruebas, la suma de cuyos resultados dará lugar a la variable genérica $UPDRSIII$. La puntuación de cada paciente en ésta escala consiste de la suma de las puntuaciones del paciente en cada una de las pruebas $X18, \ldots ,X31$. Queremos ver, a grandes rasgos, el aspecto que tienen éstas variables. Con éste fin mostramos el gráfico siguiente:

```{r, echo = FALSE, fig.height=7}
pruebas = data[1:80, 12:38]

longer_pruebas <- pruebas %>%
  pivot_longer(X18:X31, names_to = "prueba", values_to = "resultado")

ggplot(data = longer_pruebas) +
  geom_bar(mapping = aes(x = resultado, fill = as_factor(resultado)), stat="count") +
  facet_wrap(vars(prueba), ncol = 4) +
  labs(x = "Resultado (En una escala del 0 al 4)", y = "Frecuencia", title = "Resultados de las pruebas \n del test UPDRSIII de todos los pacientes") +
...
Los resultados de las pruebas son similares entre ellos por lo que únicamente analizaremos en profundidad la variable $X21$, que consideramos de interés ya que representa el temblor de acción o postural, que consideramos que es una de las variables que más podría influir en la calidad de vida del paciente.
```{r, echo = FALSE, message=FALSE, warning=FALSE}
X21 = data[1:80, 19:20]
X21RUE <- table(X21$`X21-RUE`)
X21LUE <- table(X21$`X21-LUE`)
barplot(t(cbind(X21RUE, X21LUE)), beside = T, col = c("#69b3a2", "#d9a630"), legend.text = TRUE)
```
Por tanto vemos que hay unos 35 pacientes enfermos de Parkinson o que padecen trastorno del sueño REM tanto con puntuaciones de 0 como de 1 sobre 4 para la prueba de temblor de acción o postural. Esto indica que, en general, los pacientes presentan una evolución leve de la enfermedad en éste aspecto. Sin embargo, hay unos 5-10 pacientes que tienen síntomas más graves y que podrían dificultar la vida cotidiana.

Volviendo a los diagramas de barras de las pruebas, vemos también que en su mayoría las puntuaciones de los pacientes en cada prueba se distribuyen decrecientemente, en general, el mayor número de pacientes es valorado con un 0 y el número de pacientes decrece a medida que aumenta la puntuación. Esta distribución se debe a que, como el objetivo del estudio es determinar biomarcadores prematuros de la enfermedad del Parkinson, los pacientes analizados se caracterizan por no estar en un estado avanzado del desarrollo de los síntomas de la enfermedad, e incluso tenemos en la muestra los pacientes con Transtorno del sueño REM, que ya hemos visto que tienden a tener valores inferiores en este test. 

Por tanto, podemos ver si hay un cambio significativo al eliminar a estos pacientes de la muestra.

```{r, echo = FALSE, fig.height=7}
pruebas = data[1:30, 12:38]
...
ggplot(data = longer_pruebas) +
  geom_bar(mapping = aes(x = resultado, fill = as_factor(resultado)), stat="count") +
  facet_wrap(vars(prueba), ncol = 4) +
  labs(x = "Resultado (En una escala del 0 al 4)", y = "Frecuencia", title = "Resultados de las pruebas \n del test UPDRSIII de los pacientes con Parkinson") +
  theme(plot.title = element_text(family = "serif",              
                                  face = "bold",                 
                                  color = 1,                     
                                  size = 15,                     
...
Y como podemos ver, los resultados tienden a concentrarse menos en el 0 y están mas distribuidos, aunque apenas tenemos presencia de los valores mas elevados del test.

En particular, podemos ver que las variables \textit{X29} y \textit{X23-LUE} presentan un comportamiento diferente cuando consideramos solo al grupo con Parkinson. 
...
p1 = ggplot(pruebas) +
  geom_bar(mapping = aes(x = pruebas$X29,fill = as_factor(pruebas$X29)), stat="count")+
  theme(legend.position = "none") +
    labs(x = "Resultado (En una escala del 0 al 4)", y = "Frecuencia", title = "                           X29")+
  scale_fill_brewer(palette="Set2")

p2 = ggplot(pruebas) +
  geom_bar(mapping = aes(x = pruebas$`X23-LUE`,fill = as_factor(pruebas$`X23-LUE`)), stat="count")+
  theme(legend.position = "none") +
    labs(x = "Resultado (En una escala del 0 al 4)", title = "                           X23-LUE", y ="")+
  scale_fill_brewer(palette="Set2")

p1 + p2
```

Entonces, mirando el primer gráfico vemos que dentro del grupo de pacientes con Parkinson tenemos una gran cantidad de ellos que presentan un valor de 1 en la variable \textit{X29}, que se trata de la prueba que evalua la forma de andar, es decir que la gran mayoría presentan un leve problema al andar. Y en el segundo gráfico vemos que la gran mayoría presentan un valor de almenos 1 en la variable \textit{X23-LUE}, y si recordamos el significado de esta variable, nos está diciendo que la mayoría presentan dificultades a la hora de golpear los dedos índice y pulgar, siendo esta una acción apenas posible de realizar en algunos casos (pacientes con valor 4 en esta variable) 
...
par(mfrow=c(1,3))
park <- colMeans(data[data$disease == "PD", 39:62])
rbd <- colMeans(data[data$disease == "RBD", 39:62])
control <- colMeans(data[data$disease == "HC", 39:62])
...
```{r, echo = FALSE, fig.align = "center", message = FALSE, warning=FALSE, results=FALSE, fig.height=4}
data_speech <- data[,39:62]
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.align = "center", results=FALSE, fig.height=4}
data_speech2 <- data[,c(3,4,39:62)]
data_speech2$gender = factor(data_speech2$gender, levels = c("F", "M"), labels = c(0,1))
data_speech2 <-lapply(data_speech2,as.numeric)
data_speech2_scaled <- scale(as_tibble(data_speech2))

hkmeans_cluster3 <- hkmeans(x = data_speech2_scaled, hc.metric = "binary",
 hc.method = "complete", k = 4)
fviz_cluster(object = hkmeans_cluster3, pallete = "jco", repel = TRUE) +
 theme_bw() + labs(title = "Hierarchical k-means Cluster plot")

table(hkmeans_cluster3$cluster, data$disease, dnn = list("clusters", "enfermedad padecida"))

data_speech2_scaled <- data_speech2_scaled[hkmeans_cluster3$cluster != 2,]
#AQUÍ HEMOS QUITADO LOS DATOS DEL CLUSTER SEPARADO QUE SON "OUTLIERS"

hkmeans_cluster4 <- hkmeans(x = data_speech2_scaled, hc.metric = "binary",
 hc.method = "complete", k = 3)
fviz_cluster(object = hkmeans_cluster4, pallete = "jco", repel = TRUE) +
 theme_bw() + labs(title = "Hierarchical k-means Cluster plot")

table(hkmeans_cluster4$cluster, data[hkmeans_cluster3$cluster != 2,]$disease, dnn = list("clusters", "enfermedad padecida"))
```

```{r, echo = FALSE, fig.show = "hold", fig.align = "center", message = FALSE, warning=FALSE , results=FALSE, fig.height=4}
